  ...
  public:
    ...
    explicit MLP(std::vector<size_t> units_per_layer, float lr = .001f) :
      ...
      lr(lr) {
        for (size_t i = 0; i < units_per_layer.size() - 1; i++) {
          ...
          Matrix<T> W  = matrix::mtx<T>::randn(out_channels, in_channels);
          weight_matrices.push_back(W);

          Matrix<T> b  = matrix::mtx<T>::randn(out_channels, 1);
    ...
    void backprop(Matrix<T> target) {
      ...
      for(int i = weight_matrices.size() - 1 ; i >= 0; --i) {
        ...
        auto d_outputs = activations[i+1].applyFunction(d_sigmoid);
        auto gradients = error.multiplyElementwise(d_outputs);
